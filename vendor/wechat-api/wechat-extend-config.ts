...
export function loadWechatExtendConfig(): WechatExtendConfig {
  const p = path.join(os.homedir(), ".papergirl", "wechat-extend.md");
  try {
...
function resolveCredentialSource(
  ...
  throw new Error(
    ...
    "Set via ~/.papergirl/wechat-extend.md account config, environment variables, or ~/.papergirl/wechat.env." +
    partialHint
  );
}

export function loadCredentials(account?: ResolvedAccount): LoadedCredentials {
  const configEnv = loadEnvFile(path.join(os.homedir(), ".papergirl", "wechat.env"));
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`~/.papergirl/wechat.env (${prefixedKeyLabel})`, configEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
    );
  }

  sources.push(
    buildCredentialSource("process.env", process.env, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("~/.papergirl/wechat.env", configEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
  );
