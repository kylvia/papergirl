...
- 读 `beats.yaml`：账号定位、赛道清单与**信息源配置**（`sources:` 决定第 1 / 1.5 步扫哪些源）
...
- 每个赛道跑一次 last30days（宽口径 query，平台取 beats.yaml `sources.last30days.platforms`），**必须带 `--store`**（把结构化证据沉淀进跨期库，供趋势判断），**只用 `--emit=compact`，不要用 `--emit md`**（后者是禁用的调试模式）。其中 X 源没有登录态就直接跳过（免费源 Reddit / HN / GitHub 零配置足够托底）；platforms 里没列的源不扫
- 产出候选清单：每条 = 主题 + 跨平台热度证据（哪个平台、多少互动）+ 时效判断（上升期/已过峰）

## 1.5 补盲源扫描（WebSearch，补 last30days 覆盖盲区）

读 beats.yaml `sources.web_supplement`。**`enabled: false` 或该段缺失就跳过本步**，直接用第 1 步候选继续。启用时按其配置补盲（本账号补的是国内 AI 资讯盲区，具体见配置里的 `rationale`）：

- 用 **WebSearch** 定向扫 `domains` 列出的源近 ~7 天热点，query 参考 `query_templates`（按当期赛道调整），2-3 条够了别铺开
- **这 ~7 天是「撒网 / 升温判断」窗口，不是新鲜度标准**：宽扫只为发现候选 + 看清事件链与上升期；能不能选中仍受第 2 步「≤2 天新闻由头」地板约束。别把扫到的几天前旧账当成「7 天内都能写」（宽扫窄选）
- 命中的主题**并进第 1 步候选清单**，同格式（主题 + 热度证据 + 时效），并标 **`源:<label>`**（label 取配置，本账号即 `源:国内`）供决策记录追溯。补盲源通常给不出 upvote 数，热度用代理信号（多家转载 / 头条置顶 / 官方发布会 / 上榜）
- **不设补盲源配额**：这步只保证补盲候选「进得了池子」，选不选仍由第 2 步 HKR + 差异度 + 素材厚度公平打分定。宁可本期无合格补盲候选，也不降标硬塞（凑配额会腐蚀「读者价值优先」）
- 事实纪律加码：补盲源多为二手转述、失真率高，WebSearch 摘要只作选题线索，**选中后第 3 步深研必须回原文/官方核实**——尤其融资额、参数、「首个/最强」这类数字，锚不到来源就弃
